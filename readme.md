...
93. NAT Gateway Hands on Labs.
Stop/terminate previous NAT Instance.
On CLI (EC2 instant connect) do curl google.com and nothing should be returned.
On console, go to Route table and refresh, you should see the one with destination as 0.0.0.0/0 with a Blackhole status. (The route leads to nothing as the previous NAT instance cant be reached now)
So click NAT Gateway → Create NAT gateway → Demo NATGW, subnet = PublicSubnetA, connectivity = public, Allocate Elastic IP → Tags = Name, DemoNATGW → Create NAT Gateway.

Now go edit private route table → Edit the blackhole status one and change target to NAT gateway and select the one you made (DemoNATGW) → Save changes.
 Now go back to EC2 direct connect CLI → curl google.com and it should return some info showing its working! Same with ping google.com
And here we didn't have to specify any security group rules, just create NAT gateway, sign it into a subnet and then edit route table. Much simpler than a NAT instance.
Now we can do sudo yum install to update the OS on the instance without having to make it public and at risk to outsiders. 
If we wanted to make it highly available we just make another NAT GW in another AZ and edit the route table.

94. NACL & SG hands on labs
Go to bastion host, EC2 Instance connect → Connect → sudo yum install -y httpd → start web server with sudo systemctl enable httpd → sudo systemctl start httpd → sudo su → echo “hello world” > var/www/html/ndex.html

Now make sure bastion host has http enabled in sg so edit inbound rules → Add rule for type: HTTP source: 0.0.0.0/0 → Save
Now copy bastion host public IPv4 address and paste into browser and it should show Hello World aka success!

Now lets look at default NACL, edit inbound rules → add new rule, rule number = 80, type:HTTP, source 0.0.0.0/0, deny → Save
So now we refresh our browser and its stuck on infinite loop and wont load as the NACL has blocked the request.
But say we edit the rules and change the rule number from 80 to 140 and still on deny. → save
This time the browser works when we save, why? Because rule 140 happens after rule 100 has a higher precedence despite 140 having an explicit deny.

Outbound rules → If we edit the rule 100 on the NACL outbound rule to deny, and we save.
Go back to browser, refresh the page and you're stuck on infinite load bc NACL hasn't explicitly allowed return traffic.
